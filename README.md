...
3. **Model Training** (`train_models.py`): Splits the encoded arrays into training and testing sets, trains a standard CART Decision Tree, evaluates its accuracy, precision, recall, and f1 score; and serializes the model into a `.joblib` file.
3.5 **Plotting performance curves** (`generate_curves`.py): Plot the loss/accuracy over epochs/example numbers curves for each model.
